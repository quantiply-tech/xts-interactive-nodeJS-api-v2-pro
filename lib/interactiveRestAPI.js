...
module.exports = class XTSInteractive {
   ...
   */
  constructor(url, clientID = false) {
    this.url =
  ...
  async placeOrder(reqObject) {
    if (this.clientID) reqObject = { clientID: this.clientID, ...reqObject };
    try {
      await this.checkLoggedIn();
      let response = await request.processRequest(
        "POST",
  ...
  async cancelOrder(reqObject) {
    ...
    if (this.clientID) requestUrl += "&clientID=" + this.clientID;
    try {
      await this.checkLoggedIn();
      let response = await request.processRequest(
        "DELETE",
   ...
   */
  async getOrderBook() {
    let requestUrl = this.url + settings.restApi.orders;
    if (this.clientID) requestUrl += "?clientID=" + this.clientID;
    try {
      await this.checkLoggedIn();
      let response = null;
  ...
  async getTradeBook() {
    try {
      let requestUrl = this.url + settings.restApi.trade;
      if (this.clientID) requestUrl += "?clientID=" + this.clientID;
      await this.checkLoggedIn();
      let response = null;
      response = await request.processRequest(
